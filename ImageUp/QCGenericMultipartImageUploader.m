...
@interface QCGenericMultipartImageUploader()
- (id)initWithImage:(NSImage *)inImage
		   uploader:(QCImageUploaderPlugin *)inUploader
			   chat:(AIChat *)inChat;
- (void)uploadImage;
@end

@implementation QCGenericMultipartImageUploader
+ (id)uploadImage:(NSImage *)image forUploader:(QCImageUploaderPlugin *)uploader inChat:(AIChat *)chat;
{
...
//	}
	...
	[body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"; filename=\"image\"; private=\"false\"\r\n", self.fieldName] dataUsingEncoding:NSUTF8StringEncoding]];
